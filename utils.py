...
def compute_segmentation_accuracy(outputs, targets) -> float:
    """
    Calculate pixel-wise accuracy for segmentation tasks.

    Args:
        outputs: Model predictions [B, C, H, W]
        targets: Ground truth segmentation masks [B, 1, H, W] or [B, H, W]
    ...
    if targets.dim() == 4 and targets.size(1) == 1:
        targets = targets.squeeze(1)  # Remove channel dim if present

    # Make sure targets are properly formatted (0-indexed)
    if targets.min() == 1 and targets.max() <= outputs.size(1):
        targets = targets - 1  # Convert from 1-indexed to 0-indexed

    # Count correct pixels across entire batch
    correct = (predicted == targets).sum().item()
