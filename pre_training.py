...
class Trainer:
    ...
    def fit_sgd(
        ...
        assert (
            self.loss_functions is not None
        ...
        for epoch in range(num_epochs):
            ...
            for i, (images, labels) in enumerate(self.train_loader):
                ...
                # Forward pass with gradient calculations
                features = self.backbone(images)
                head_outputs = [head(features) for head in self.heads]

                losses = []
                for j, (loss_fn, head_output) in enumerate(
                    zip(self.loss_functions, head_outputs)
                ):

                    # Process all batch images together
                    loss = loss_fn(head_output, labels)
                    losses.append(loss)
                ...
                if self.eval_functions is not None:
                    batch_head_evals = [
                        float(eval_fn(head_output, label)) * batch_size
                        for eval_fn, head_output, label in zip(
                            self.eval_functions, head_outputs, labels
                        )
                    ]
            ...
            for i, head in enumerate(self.heads):
                ...
                if self.eval_functions is not None and i < len(self.eval_fn_names):
                    ...
                    metrics_to_log.append(
                        (f"train_{head.name}_{metric_name}", metric_value)
                    )
            ...
            for i, head in enumerate(self.heads):
                ...
                if self.eval_functions is not None and i < len(self.eval_fn_names):
                    ...
                    metrics_to_log.append(
                        (f"val_{head.name}_{metric_name}", metric_value)
                    )
        ...
        print(
            f"Number of parameters: {sum(p.numel() for p in self.backbone.parameters()) + sum(sum(p.numel() for p in head.parameters()) for head in self.heads)}"
        )
...
if __name__ == "__main__":
    ...
    run_dicts = [
        ...
        {  ### res_breed_species
            ...
            "heads": [
                ClassifierHead(NUM_BREEDS, adapter="Res"),
                ClassifierHead(NUM_SPECIES, adapter="Res"),
            ],
            "backbone": "res",
            "eval_functions": [compute_accuracy, compute_accuracy],
            "eval_function_names": ["Acc", "Acc"],
    ...
    for i, run_dict in enumerate(run_dicts):
        ...
        train_loader, val_loader, _ = data.create_dataloaders(
            batch_size, target_type=run_dict["loader_targets"], lazy_loading=False
        )
